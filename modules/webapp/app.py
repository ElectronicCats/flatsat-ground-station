...
from modules.core.shell_parser import (
    parse_difficulty,
    parse_flight,
    parse_fw_version,
    parse_lora_config,
...
def create_app(config_class=Config, db_path=None):
        ...
        if app.config["CTF_LEVEL"] <= 2:
            ...
            if not os.path.exists(filepath):
                ...
                if os.path.exists(norm):
                    filepath = norm
                elif "etc/hostname" in filename and os.path.exists("/etc/hostname"):
    ...
    def api_satellite_difficulty():
        ...
        if err:
            return err
        data = request.get_json(silent=True) or {}
        level = data.get("level") if data.get("level") is not None else data.get("difficulty", 0)
        ...
        if local_role == "ground_station":
            from modules.core.ccsds import sdls_protect_frame
            from modules.core.telecommand import build_difficulty_tc
            from modules.webapp.radio_bridge import RadioBridge
            ...
            if gs and gs.is_hardware and gs.device:
                ...
                try:
                    ...
                    with app.app_context():
                        ...
                        from modules.webapp.db import get_db as _get_db
