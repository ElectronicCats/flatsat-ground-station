...
def sdls_protect_frame(frame: bytes, difficulty: int) -> bytes:
    ...
    if len(frame) < min_size:
        raise ValueError(
            f"Frame too short for SDLS protection: expected at least {min_size} bytes, got {len(frame)} bytes"
        )
    payload_start = CCSDS_HDR_SIZE + CCSDS_SEC_HDR_SIZE
    payload_end = len(frame) - CCSDS_CRC_SIZE
    if payload_end < payload_start:
        raise ValueError(f"Malformed frame: payload end ({payload_end}) before payload start ({payload_start})")
